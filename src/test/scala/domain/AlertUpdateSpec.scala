...
class AlertUpdateSpec extends AnyWordSpec with Matchers {
  "AlertUpdate.apply" should {
    "create a AlertUpdate from valid JSON string" in {
      val json = """{"name": "John", "age": 30}"""
      val doc = AlertUpdate(json)

      doc shouldBe a[AlertUpdate]
    }
